  ...
  for (Hand hand : leap.getHands ()) {
    for (Finger finger : hand.getFingers()) {
      switch(finger.getType()){
        case 1:
        image(cursor,finger.getPosition().x-60,finger.getPosition().y-60);
        if(finger.getPosition().y > height*3/4 - 90){
            position = 0;
    ...
    for (Finger finger : hand.getFingers()) {
     switch(finger.getType()){
       case 1:
        ...
        //noFill();
        //ellipse(finger.getPosition().x,finger.getPosition().y,70,70);
        image(cursor,finger.getPosition().x - 60,finger.getPosition().y-60);
        if(finger.getPosition().x > 393 &&
          finger.getPosition().x < 454 &&
          finger.getPosition().y < 393 &&
          finger.getPosition().y > 250){
            countdecrease = 1;
            position = 3;
            fill(0,0,0, 180);
            rect(finger.getPosition().x-40,finger.getPosition().y+160,125,240,20);
            fill(255);
            text(drinkName + "\nRM" + drinkPrice + "\n" + realCount,finger.getPosition().x-38,finger.getPosition().y+90);        }
        else {
          position = 0;
        }
        if(finger.getPosition().x > 493 &&
          finger.getPosition().x < 572 &&
          finger.getPosition().y < 393 &&
          finger.getPosition().y > 250){
            countdecrease = 1;
            position = 4;
        ...
        if(finger.getPosition().x > 593 &&
          finger.getPosition().x < 672 &&
          finger.getPosition().y < 393 &&
          finger.getPosition().y > 250){
            countdecrease = 1;
            position = 5;
        ...
        if(finger.getPosition().x > 693 &&
          finger.getPosition().x < 772 &&
          finger.getPosition().y < 393 &&
          finger.getPosition().y > 250){
            countdecrease = 1;
            position = 6;
        ...
        if(finger.getPosition().x > 793 &&
          finger.getPosition().x < 872 &&
          finger.getPosition().y < 393 &&
          finger.getPosition().y > 250){
            countdecrease = 1;
            position = 7;
            ...
            fill(255);
            text(drinkName + "\nRM" + drinkPrice + "\n" + realCount,finger.getPosition().x-38,finger.getPosition().y+90);    }
        if(finger.getPosition().x > 993){
            position = 8;
            text("SHOW CART\n",finger.getPosition().x-18,finger.getPosition().y+75);
            if(hand.getPinchStrength() > .90f){
            realCount = 1;
            countdecrease = 1;
          }
        }
  ...
  for (Hand hand : leap.getHands ()) {
    for (Finger finger : hand.getFingers()) {
      switch(finger.getType()){
        ...
        image(ico,finger.getPosition().x,finger.getPosition().y);

        if(finger.getPosition().y < 230 && finger.getPosition().x > 453 && finger.getPosition().x < 700){
            countdecrease = 1;
            returnPage = 1;
            text("Return\n" + realCount,finger.getPosition().x-8,finger.getPosition().y+18);
            fromPage = 2;
        }
        else {
          returnPage = 0;
        ...
        if(finger.getPosition().y > 500 && finger.getPosition().x > 453 && finger.getPosition().x < 700){
            returnPage = 1;
            fromPage = 99;
            confirm = true;
            if(alpha3 > 150){alpha3-=delta;}
            //hand.getPinchStrength() > .90
            //hand.getPinchStrength() > .90
            if(hand.getPinchStrength() > .90f){
              realCount = 1;
              countdecrease = 1;
        ...
        //quantity
        if(finger.getPosition().x < 353 &&
          finger.getPosition().y > 500){
            countQdecrease = 1;
            text("Decrease quantity " + realCountQ,finger.getPosition().x-8,finger.getPosition().y+18);
        } else {
          countQdecrease = 0;
        }
        if(finger.getPosition().x < 353 &&
          finger.getPosition().y < 320){
            countQincrease = 1;
            text("Increase quantity\n" + realCountQ,finger.getPosition().x-8,finger.getPosition().y+18);
        } else {
          countQincrease = 0;
        }

        //size
        if(finger.getPosition().x > 862 &&
          finger.getPosition().y > 520){
            countSdecrease = 1;
            text("Decrease size\n" + sizeName,finger.getPosition().x-8,finger.getPosition().y+18);
        } else {
          countSdecrease = 0;
        }
        if(finger.getPosition().x > 862 &&
          finger.getPosition().y < 320){
            countSincrease = 1;
            text("Increase size " + sizeName,finger.getPosition().x-8,finger.getPosition().y+18);
        } else {
          countSincrease = 0;
        }
  ...
  for (Hand hand : leap.getHands ()) {
    for (Finger finger : hand.getFingers()) {
      ...
      if(realCount == 0){ //confirm
        ...
        image(cursor,finger.getPosition().x - 60,finger.getPosition().y-60);
        /*if(finger.getPosition().y > 560 && finger.getPosition().x > 426){
            countdecrease = 1;
            returnPage = 1;
            text("Return\n" + realCount,finger.getPosition().x-8,finger.getPosition().y+68);
            fromPage = 3;
          }*/


        if(finger.getPosition().x < 426 ){
            countdecrease = 1;
            returnPage = 4;
            text("Pinch here\nto confirm",finger.getPosition().x-8,finger.getPosition().y+68);
            fromPage = 3;
            confirm = true;
            if(hand.getPinchStrength() > .90f){
              realCount = 1;
...
public void leapOnSwipeGesture(SwipeGesture g, int state){
  ...
  static public void main(String[] passedArgs) {
    String[] appletArgs = new String[] { "main1" };
    if (passedArgs != null) {
